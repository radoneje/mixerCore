...
void CHttp::init(int port, Ccmd *pCmd)
{
    ...
    svr.Get(
        ...
        [&](const httplib::Request &req, httplib::Response &res)
        {
            ...
            const std::string islooped = req.get_param_value("islooped");

            res.set_content(pCmd->loadPresVideo(eventid, fileid, url, islooped == "true"), "application/json");
    ...
    svr.Get(
        ...
        [&](const httplib::Request &req, httplib::Response &res)
        {
            ...
            const std::string fileid = req.get_param_value("fileid");
            const std::string islooped = req.get_param_value("islooped");
            CConfig::log("/mixer/activatePresVideo", eventid,fileid, islooped );

            res.set_content(pCmd->loopedPresVideo(eventid, fileid, islooped), "application/json");
