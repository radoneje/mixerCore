   ...
   // stream_ctx[i].enc_ctx = enc_ctx;
    ...
    long long i=0;
       ...
       locker->unlock();
        ...
        frame->pts +=   av_rescale_q( 1, enc_ctx->time_base, out_stream->time_base)*1000;
            if(frame->pts<0) {
                frame->pts = 0;
            ...
            std::cout<< frame->pts<< " " <<dts  << " " <<std::endl;

            if(dts<(frame->pts))
            {
                std::cout<< (frame->pts)-dts<<" sleeo"<<std::endl;
                av_usleep((frame->pts)-dts );
            }
